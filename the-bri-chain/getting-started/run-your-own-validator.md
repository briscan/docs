...
2. Download one of the example `.env` files located at the [examples folder](https://github.com/bri/bri-network/tree/master/scripts/examples).
...
```text
$ wget -O quickstart.sh https://raw.githubusercontent.com/briscan/BRI/master/scripts/quickstart.sh
$ chmod 777 quickstart.sh
$ wget -O .env https://raw.githubusercontent.com/briscan/BRI/master/scripts/examples/.env.<ROLE>.example
$ ./quickstart.sh
```
...
To run the parity client for the FuseNetwork you first have to pull the image from [DockerHub](https://hub.docker.com/r/bri/node).
...
```text
$ docker pull bri/node
$ docker run bri/node --help
...
$ docker run \
    --detach \
    --name bri \
    --volume $(pwd)/bri/database:/data \
    --volume $(pwd)/bri/config:/config/custom \
    -p 30303:30300/tcp \
    -p 30303:30300/udp \
    -p 8545:8545 \
    -p 8546:8546 \
...
If you have no account already or want to create a new one for this purpose checkout [this section](https://github.com/bri/bri-network/blob/master/README.md#create-new-account).
...
One is the [validator-app](https://github.com/bri/bri-network/tree/master/app)
...
Second one is the [bridge-oracle](https://github.com/bri/bridge-oracle)

```text
$ wget -O docker-compose.yml https://raw.githubusercontent.com/bri/bridge-oracle/master/docker-compose.keystore.yml
...
In relation to the instructions for the [validator](https://github.com/bri/bri-network/blob/master/README.md#validator) role, we use the folder called `config` to bind as _Docker_ volume to _Parity_.

Afterwards the key will be placed there and the first steps of these instructions can be skipped.

```text
$ mkdir ./config
$ docker run -ti -v $(pwd)/config/:/config/custom bri/node --parity-args account new
```
...
If you want to run a node to be used by the [blockscout explorer](https://github.com/bri/blockscout/tree/bri) run the following command:
...
All roles should also run a [Ethereum Network Intelligence API](https://github.com/bri/eth-net-intelligence-api) app as well, in order to connect themselves as part of the network and be viewed by the [health](https://status.briscan.io/) service
